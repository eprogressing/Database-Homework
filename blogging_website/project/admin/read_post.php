...
<section class="read-post">
   ...
   <form method="post">
      ...
      <div class="status" style="background-color:<?php if($fetch_posts['status'] == 'active'){echo 'limegreen'; }else{echo 'coral';}; ?>;">
         <?= $fetch_posts['status'] == 'active' ? '已激活' : '未激活'; ?>
      </div>
      ...
      <div class="flex-btn">
         <a href="edit_post.php?id=<?= $post_id; ?>" class="inline-option-btn">编辑</a>
         <button type="submit" name="delete" class="inline-delete-btn" onclick="return confirm('确定要删除此文章吗？');">删除</button>
         <a href="view_posts.php" class="inline-option-btn">返回</a>
      </div>
   </form>
   <?php
         }
      }else{
         echo '<p class="empty">暂无文章！ <a href="add_posts.php" class="btn" style="margin-top:1.5rem;">添加文章</a></p>';
      }
   ?>
